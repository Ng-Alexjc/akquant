...
个股事实查询会优先提取主力、超大单和大单资金流字段（兼容行标签、列名和字典字段）；在 `capital_flow` 结构中，中单、小单及无法归类的通用字段不会保留。上述三类中任意一类返回非空值，即将 `capital_flow.status` 标记为 `valid`，不再因为缺少其他分类而判定资金流向缺失。板块查询在同一次板块请求中尝试提取最多 5 只核心/龙头的今日与近 5 日表现，放入 `sector_context.core_performance`。这两块均有长度上限，不会把完整板块成分或原始资金流明细传给 LLM。
...
当前版本 `2026-08-30.v2` 默认固定注入 5 条全局硬规则，并按本次市场、板块、个股和持仓上下文检索最多 12 条相关规则。检索先比较标签/标题与当前场景的相关性，再比较规则优先级，避免无关高优先级卡片挤掉竞价、缩量、被套或次日反馈等明确命中的经验。
...
```text
http://127.0.0.1:8765/api/backtest/review-baseline?days=60
```

接口自动读取当前持仓池和观察池的 A 股日 K 数据，按收盘时可见信息模拟短线趋势基准：`close > MA5 > MA20` 且 20 日动量为正时，以权益的 10% 买入；跌破 MA20 或 20 日动量转负时卖出；最多持有 5 只，每次按 100 股整手计算，并计入手续费、印花税和滑点。

接口返回日终权益曲线以及年化收益、最大回撤、夏普、索提诺和卡玛比率。该结果用于在真实交易记录不足时建立风险指标初始值，不代表历史上实际发生过的成交，也不替代完整样本外策略回测。
