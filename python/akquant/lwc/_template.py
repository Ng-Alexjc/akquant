...
<style>
  ...
  *{box-sizing:border-box} html,body{margin:0;min-height:100%;background:var(--bg);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC",Roboto,sans-serif;}
  ...
  .trade-form{display:grid;gap:8px;padding:10px 12px;border-bottom:1px solid var(--border);background:var(--surface-soft)}.trade-form .trade-search{display:flex;gap:8px}.trade-form input{min-width:0;height:32px;padding:0 9px;border:1px solid var(--border);border-radius:7px;background:var(--surface);color:var(--text)}.trade-form .trade-search input{flex:1}.trade-fields{display:grid;grid-template-columns:1fr 1fr;gap:8px}.trade-actions{display:flex;gap:8px}.trade-actions .button{flex:1}.button.trade-buy{color:var(--up);border-color:var(--up)}.button.trade-sell{color:var(--down);border-color:var(--down)}.signal-list{padding:0}.signal-table{width:100%;min-width:1160px;table-layout:fixed}.signal-table th,.signal-table td{text-align:center;vertical-align:middle}.signal-table th{height:42px;padding:10px 8px;font-size:12px;letter-spacing:.02em}.signal-table th:first-child,.signal-table td:first-child,.signal-table th:nth-child(8){text-align:left}.signal-table .pool-symbol{width:100%;min-width:0}.signal-table .signal-evaluation{text-align:left;white-space:normal;line-height:1.55}.signal-advice{font-weight:800}.signal-advice.buy{color:var(--up)}.signal-advice.sell{color:var(--down)}.signal-advice.add{color:#d08b20}.signal-advice.hold{color:var(--muted)}
...
<main class="app">
  ...
  <section class="workspace">
    <div class="stack">
      ...
      <section class="card"><div class="card-header"><h2>交易信号</h2></div><div id="signal-list" class="signal-list"></div></section>
...
(function(){
  ...
  function signalClass(action){return action==='买入'||action==='强势买入'?'buy':action==='卖出'?'sell':action==='加仓'?'add':'hold';}
  function renderSignals(){signalHost.replaceChildren();var items=poolState.signals || [];if(!items.length){appendText(signalHost,'div','当前没有需要操作的观察标的或持仓信号。','empty');return;}var wrap=document.createElement('div');wrap.className='pool-table-wrap';var table=document.createElement('table');table.className='pool-table signal-table';var colgroup=document.createElement('colgroup');['15%','9%','8%','8%','10%','9%','9%','24%','8%'].forEach(function(width){var col=document.createElement('col');col.style.width=width;colgroup.appendChild(col);});table.appendChild(colgroup);var headers=['股票名称','代码','趋势','评分','上涨概率','压力位','支撑位','评价','操作建议'];var thead=document.createElement('thead');var headRow=document.createElement('tr');headers.forEach(function(label){var th=document.createElement('th');th.textContent=label;headRow.appendChild(th);});thead.appendChild(headRow);table.appendChild(thead);var body=document.createElement('tbody');items.forEach(function(signal){var row=document.createElement('tr');var nameCell=document.createElement('td');nameCell.appendChild(symbolButton(signal,'name'));row.appendChild(nameCell);var codeCell=document.createElement('td');codeCell.appendChild(symbolButton(signal,'code'));row.appendChild(codeCell);tableCell(row,signal.trend_direction||signal.trend||'—',trendDirectionClass({analysis:signal}));var hasScore=signal.selection_score!==null&&signal.selection_score!==undefined&&signal.selection_score!=='',score=Number(signal.selection_score),scoreText=hasScore&&Number.isFinite(score)?score.toFixed(1):'—';tableCell(row,scoreText,'');var hasProbability=signal.up_probability!==null&&signal.up_probability!==undefined&&signal.up_probability!=='',probability=Number(signal.up_probability),probabilityText=hasProbability&&Number.isFinite(probability)?(probability*100).toFixed(1)+'%':'—';tableCell(row,probabilityText,'');tableCell(row,money(signal.resistance_price),'');tableCell(row,money(signal.support_price),'');tableCell(row,signal.evaluation||signal.reason||'—','signal-evaluation');tableCell(row,signal.action||'观望','signal-advice '+signalClass(signal.action));row.addEventListener('click',function(event){if(!event.target.closest('button'))openPoolSymbol(signal);});body.appendChild(row);});table.appendChild(body);wrap.appendChild(table);signalHost.appendChild(wrap);}
  ...
  function trendDirection(item){var analysis=item.analysis||{};return analysis.trend_direction||analysis.trend||'数据不足';}
  function trendDirectionClass(item){var direction=trendDirection(item);return direction==='上升'||direction==='偏强'?'positive':direction==='下降'||direction==='偏弱'?'negative':'';}
